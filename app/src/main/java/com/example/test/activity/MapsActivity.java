...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback, GoogleMap.OnMyLocationClickListener, GoogleMap.OnMyLocationButtonClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_map_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int id;
                ArrayList<City> delete = dBhelper.getAllWords();
                if(delete.size()==0){
                    id = 1;
                }else {
                    id = delete.get(delete.size()-1).getID()+1;
                }
                for (int i=0;i<delete.size();i++){
                    if (tv_city.getText().toString().equals(delete.get(i).getCity_Name())){
                        Toast.makeText(MapsActivity.this, "Đã có thành phố này!!!", Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
                long resultAdd = dBhelper.Insert(id,tv_city.getText().toString(),tv_country.getText().toString(),
...
//                Toast.makeText(MapsActivity.this, ""+delete.size(), Toast.LENGTH_SHORT).show();
            }
        });
